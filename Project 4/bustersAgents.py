...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dist in livingGhostPositionDistributions:
            ghostPositions.append(dist.argMax())

        # determine the closest ghost to Pacman
        closestPosition = min(ghostPositions, key=lambda pos: self.distancer.getDistance(pacmanPosition, pos))

        # determine best action that brings Pacman to the closest ghost
        bestAction = min(legal, key=lambda action: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action),closestPosition))
