...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [(PAC, OBS0), (GHOST0, OBS0), (GHOST1, OBS1)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        #get all the Conditional Probability Tables (CPTs)
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        #loop through elinmination order
        for variableEliminate in eliminationOrder:
            #join factors with current variableEliminate
            factors, newFactor = joinFactorsByVariable(factors, variableEliminate)
            #check the factor has more than one unconditioned variable
            if len(newFactor.unconditionedVariables()) > 1:
                factors.append(eliminate(newFactor, variableEliminate))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for item, prob in self.items():
            if currentCumulativeSum + prob > r:
                return item
            currentCumulativeSum += prob
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # check if ghost are in jail
        if ghostPosition == jailPosition:
            return 1 if noisyDistance is None else 0

        #checks for noisy distance is assigned
        if noisyDistance is None:
            return 0
        #determines the distance between pacman's location and ghost position using manhattan distance
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        #loops through and for each position calculate transition probability
        for position in self.allPositions:
            transitionProbability[position] = self.getPositionDistribution(gameState, position)

        #loops through but this time updates belief based on transition probability
        for position in self.allPositions:
            updateValue = 0
            #loops through previous positions in belief
            for prevPosition in self.beliefs:
                #updates value with transition probability and current belief
                updateValue += transitionProbability[prevPosition][position] * self.beliefs[prevPosition]
            newBeliefs[position] = updateValue
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        #calculate the number of particles per legal action
        particlesPerPosition = numParticles // len(legalPositions)

        #calculate remaining particles after distributing evenly
        remainderParticles = numParticles % len(legalPositions)

        #evenly separate the particles in all legal positions
        for position in legalPositions:
            self.particles.extend([position] * particlesPerPosition)

        remainderPositions = random.sample(legalPositions, remainderParticles)
        self.particles.extend(remainderPositions)
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            #increase count for current particle in belief distribution
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        updatedDistribution = DiscreteDistribution()

        #loops through each ghost position in particles
        for ghostPosition in self.particles:
            updatedDistribution[ghostPosition] += self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)

        #checks if all particles have zero weight
        if updatedDistribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            updatedDistribution.normalize()
            self.particles = []
            index = 0
            while index < self.numParticles:
                sampleParticle = updatedDistribution.sample()
                self.particles.append(sampleParticle)
                index += 1
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        positionDististribution = {}
        index = 0

        while index < len(self.particles):
            currentParticle = self.particles[index]

            #checks if current particle postion is the dictionary
            if currentParticle not in positionDististribution.keys():
                #calculate position distribution and store it
                positionDististribution[currentParticle] = self.getPositionDistribution(gameState, currentParticle)
            newParticle = positionDististribution[currentParticle].sample()

            #updates the particle with new state
